...
function addGlobalResult(a:number, b:number) {
    myResult = a + b;
    return myResult;
}
...
const addExpression = (a: number, b: number) => a + b;
...
const addWithDefault = (a: number, b: number = 1) => a + b;
...
const deepCopiedPerson = { ...person };

// Rest paramters allow you to accept an unlimited amount of arguments. 
const addAdInfinitum = (...numbers: number[]) => { // will merge all incoming parameters into an array
